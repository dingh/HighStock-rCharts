...
##
## Candle Chart
candlePlot <- highstockCandle <- function
( 
  name = "ticker", 
  data = data, 
  type = "candlestick", 
...
)
{
  ...
  height <- rChart$params$height
  ## title
  rChart$title(text = title, replace = T)  
  
  rChart$yAxis(title = list(text = "OHLC"), height =  height- 165, linewidth = 2, id = yAxis)
  rChart$series(data = toJSONArray2(data[c("date", "Open", "High", "Low", "Close")], json = F, names = F), 
                type = type, name = name, yAxis = yAxis, marker = list(radius = radius))
